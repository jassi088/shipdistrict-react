...
const FormSchema = z.object({
  shipmentDate: z.union([z.string().min(1, 'Shipment date is required.'), z.date()]),
...
export type SenderDetailInput = z.infer<typeof FormSchema>;

const AddSenderDetailPage = () => {
  const { goToRoute, goToPreviousRoute } = useRouteNavigator();

  const form = useForm<SenderDetailInput>({
  ...
  const onSubmit: SubmitHandler<SenderDetailInput> = async (values) => {
    console.log(values);
    goToRoute(Paths.CREATE_SHIPMENT + Paths.ADD_RECEIVER_ADDRESS);
  };

  return (
    <CreateShipmentLayout title="Sender Address" progress={20}>
...
export default AddSenderDetailPage;
